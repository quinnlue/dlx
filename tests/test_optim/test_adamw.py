...
# --------------------------------------------------------------------------- #
    ...
    def test_gradient_clipping(self):
        ...
        x_data = (xp.random.randn(6, 4) * 1e8).astype(xp.float32)
        w_data = xp.random.randn(4, 4).astype(xp.float32)
        ...
        print('w_clip.grad (before step):', w_clip.grad.data.flatten()[:5])
        print('w_noclip.grad (before step):', w_noclip.grad.data.flatten()[:5])
        print('w_clip.data (before step):', w_clip.data.flatten()[:5])
        print('w_noclip.data (before step):', w_noclip.data.flatten()[:5])
        print('w_data (original):', w_data.flatten()[:5])

        optim_clip.step()
        optim_noclip.step()

        # Print weights after step
        print('w_clip.data (after step):', w_clip.data.flatten()[:5])
        print('w_noclip.data (after step):', w_noclip.data.flatten()[:5])
        ...
        print(f"delta_noclip: {delta_noclip}")
